...
import { ArrowUpRight, ShieldCheck, Video, Eye, EyeOff, Mail, Lock, User } from 'lucide-react';
...
/* ════════════════════════════════════════════
   INPUT FIELD
════════════════════════════════════════════ */
const Field = ({ label, name, type = 'text', placeholder, value, onChange, icon: Icon, required, error }) => {
  ...
  return (
    <div>
      <label style={{
        ...
        color: error ? '#991b1b' : '#A08050', marginBottom: 6,
      }}>{label}</label>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 10,
        background: focused ? 'rgba(255,255,255,0.82)' : 'rgba(255,255,255,0.55)',
        border: `1.5px solid ${error ? 'rgba(220,38,38,0.45)' : (focused ? 'rgba(184,116,26,0.6)' : 'rgba(180,130,70,0.28)')}`,
        borderRadius: 12, padding: '11px 14px',
        transition: 'all 0.2s', backdropFilter: 'blur(8px)',
      }}>
        <Icon size={14} color={error ? '#dc2626' : (focused ? '#B8741A' : 'rgba(160,114,42,0.45)')} style={{ flexShrink: 0, transition: 'color 0.2s' }} />
        <input
      ...
      {error && <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: '0.7rem', color: '#dc2626', marginTop: 4 }}>{error}</p>}
    </div>
  );
};
...
const RegistrationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.terms) { setApiError('You must agree to the Terms of Service.'); return; }
    setIsLoading(true);
    try {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 440px', gap: 64, alignItems: 'center', maxWidth: 980, width: '100%' }}>
            ...
            >
              <div style={{
                ...
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#E8A020', boxShadow: '0 0 5px #E8A020', animation: 'blink 1.5s infinite' }} />
              ...
              <div style={{ display: 'flex', gap: 0, marginTop: 36, paddingTop: 24, borderTop: '1px solid rgba(180,130,70,0.2)' }}>
                {[
                  ...
                  <div key={lbl} style={{
                    ...
                    borderRight: i < 2 ? '1px solid rgba(180,130,70,0.2)' : 'none',
                  }}>
            ...
            >
              <div style={{
                ...
                <AnimatePresence>
                  {apiError && (
                    <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      style={{ marginBottom: 14, padding: '10px 14px', background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 10, fontSize: '0.8rem', color: '#991b1b', fontWeight: 500 }}>
                      {apiError}
                    </motion.div>
                  )}
                  {apiSuccess && (
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
                  <Field label="Full Name" name="name" placeholder="John Doe" value={formData.name} onChange={handleChange} icon={User} required />
                  ...
                  <Field label="Confirm Password" name="confirmPassword" type="password" placeholder="••••••••" value={formData.confirmPassword} onChange={handleChange} icon={Lock} required error={errors.confirmPassword} />
